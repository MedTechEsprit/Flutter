...
class DoctorService {
  ...
  Future<Map<String, dynamic>> purchaseDoctorBoost(String boostType) async {
    try {
      ...
      await _revenueCatService.purchasePackageById(packageId);

      // L'achat a réussi ! Calculer la date d'expiration selon le type de boost
      ...
      final expiresAt = now.add(Duration(days: boostDays));

      // Essayer de synchroniser avec le serveur en arrière-plan (non bloquant)
      try {
        await Future.delayed(const Duration(seconds: 2));
        final serverResult = await verifyDoctorBoostLatestPayment();
        if (serverResult['isActive'] == true) {
          return serverResult;
        }
      } catch (_) {
        // Si le serveur échoue, on retourne quand même un statut actif local
        print('⚠️ Sync serveur échouée, utilisation du statut local');
      }

      // Retourner un statut actif immédiatement basé sur l'achat réussi
      return {
        'isActive': true,
        'boostType': boostType,
        'expiresAt': expiresAt.toIso8601String(),
        'remainingDays': boostDays,
  ...
  Future<Map<String, dynamic>> verifyDoctorBoostLatestPayment() async {
    try {
      ...
      print('DEBUG: Synchronisation boost pour le médecin ID: $doctorId');
      final response = await http
  ...
  String _resolveBoostPackageId(String boostType) {
    ...
    switch (type) {
      ...
      case '15d':
      case 'boost_15d':
        // Utilise boost_day pour le slot de 15 jours (ou créez boost_15d dans RC)
        return RevenueCatConstants.doctorBoost24hPackageId; 
      case 'month':
      case 'monthly':
      case '30d':
