...
class _DoctorProfileScreenState extends State<DoctorProfileScreen> {
  ...
  Future<void> _loadDoctorProfile() async {
    try {
      ...
      if (_doctorId != null) {
        ...
        try {
          // 1. Vérifier le statut côté serveur
        ...
        // 2. Si le serveur ne dit pas "actif", vérifier localement via RevenueCat SDK
        if (boostStatus == null || boostStatus['isActive'] != true) {
          try {
            final localBoost = await _revenueCatService
                .getActiveBoostFromLocal();
            if (localBoost != null && localBoost['isActive'] == true) {
              boostStatus = localBoost;
              print('✅ Boost récupéré depuis le cache local RevenueCat');
            }
          } catch (_) {}
        }
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Expanded(child: Text('Load error: $e')),
              ],
            ),
            backgroundColor: const Color(0xFFFF6B6B),
            behavior: SnackBarBehavior.floating,
  ...
  Future<void> _refreshBoostData() async {
    ...
    try {
      ...
      // 1. Essayer la synchro serveur
      try {
        boostStatus = await _doctorService.verifyDoctorBoostLatestPayment();
      } catch (_) {
        // Si le serveur échoue, on essaie de lire le statut passif
        try {
          boostStatus = await _doctorService.getDoctorBoostStatus(_doctorId!);
        } catch (_) {}
      }
      ...
      if (boostStatus == null || boostStatus['isActive'] != true) {
        try {
          final localBoost = await _revenueCatService.getActiveBoostFromLocal();
          if (localBoost != null && localBoost['isActive'] == true) {
            boostStatus = localBoost;
            print(
              '✅ Boost récupéré depuis le cache local RevenueCat (refresh)',
            );
          }
        } catch (_) {}
      }
  ...
  Future<void> _activateBoost(String boostType) async {
    ...
    // Si un boost est déjà actif, empêcher l'achat
    if (_boostStatus?['isActive'] == true) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'You already have an active boost. Wait for it to expire.',
          ),
    ...
    try {
      ...
      // Mettre à jour immédiatement le statut local pour bloquer les boutons
      setState(() {
        _boostStatus = result;
      });
      ...
      _showSuccessDialog(
        'Boost activated!',
        'Congratulations! Your profile is highlighted for ${result['remainingDays'] ?? '?'} days. '
            'You will receive more patient requests.',
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _verifyBoostPayment() async {
    ...
    try {
      ...
      // 1. Restaurer les achats auprès de RevenueCat
      ...
      if (isActive) {
        _showSuccessDialog(
          'Sync successful',
          'Your boost was restored successfully. Your profile is active.',
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No active boost found.'),
            backgroundColor: Colors.orange,
            behavior: SnackBarBehavior.floating,
          ),
        );
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Text(
                  isAvailable
                      ? 'Profile enabled - You are online'
                      : 'Profile disabled - You are offline',
                ),
              ],
            ),
                  ...
                  FadeInSlide(
                    ...
                    child: _buildInfoCard(
                      ...
                      children: [
                        _contactRow(
                          Icons.email_rounded,
                          _doctorData?['email'] ?? 'No email',
                          AppColors.softGreen,
                        ),
                        const Divider(height: 24, indent: 46),
                        _contactRow(
                          Icons.phone_rounded,
                          _doctorData?['telephone'] ?? 'No phone',
                          AppColors.lightBlue,
                        ),
                        if (_doctorData?['numeroOrdre'] != null) ...[
                          ...
                          _contactRow(
                            Icons.badge_rounded,
                            'License No.: ${_doctorData!['numeroOrdre']}',
  ...
  Widget _buildBoostCard() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0xFFFDBB2D).withOpacity(0.1)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Text(
            isBoostActive
                ? 'You already have an active boost. You can buy a new one after ${_formatDate(expiresAt)}.'
                : 'Activate a boost to become a suggested doctor.',
            style: TextStyle(
              color: isBoostActive
                  ? AppColors.softGreen
                  : AppColors.textSecondary,
              fontSize: 13,
              fontWeight: isBoostActive ? FontWeight.w600 : FontWeight.normal,
            ),
          ),
          if (isBoostActive) ...[
            const SizedBox(height: 4),
            ...
            Column(
              children: _boostPlans.map((plan) {
                ...
                final isCurrentActive =
                    isBoostActive && _boostStatus?['boostType'] == boostType;
                  ...
                  child: Row(
                    children: [
                      ...
                      SizedBox(
                        height: 40,
                        child: ElevatedButton(
                          onPressed: (isLoadingThis || isBoostActive)
                              ? null
          ...
          // Bouton Synchroniser supprimé car la vérification se fait automatiquement après achat
          const SizedBox(height: 4),
        ],
      ),
    );
...
class _DoctorProfileEditScreenState extends State<_DoctorProfileEditScreen> {
  ...
  Future<void> _save() async {
    ...
    if (nom.isEmpty || prenom.isEmpty || specialite.isEmpty) {
      ScaffoldMessenger.maybeOf(context)?.showSnackBar(
        const SnackBar(
          content: Text('First name, last name, and specialty are required.'),
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              _field('Last name', _nomController, Icons.person_outline_rounded),
              _field('First name', _prenomController, Icons.person_rounded),
              _field('Phone', _telephoneController, Icons.phone_rounded),
              _field(
                'Specialty',
                _specialiteController,
                Icons.medical_services_rounded,
              ),
              _field('Clinic', _cliniqueController, Icons.business_rounded),
              _field(
                'Office address',
                _adresseController,
