...
class _AddGlucoseScreenState extends State<AddGlucoseScreen>
  ...
  void _syncBleState() {
    final connected =
        BleService.instance.isConnected || BleService.instance.isNotifying;
    setState(() {
  ...
  Future<void> _saveManualReading() async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Mesure enregistrée avec succès'),
  ...
  Future<void> _saveGlucometerReading() async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Mesure du glucomètre enregistrée'),
          backgroundColor: AppColors.statusGood,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(10),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [_buildManualTab(), _buildGlucometerTab()],
      ),
    );
  }
            ...
            child: Column(
              children: [
                ...
                DecoratedBox(
                  decoration: BoxDecoration(
                    color: _isConnected
                        ? AppColors.softGreen.withOpacity(0.12)
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                icon: _isSaving
                    ...
                    : const Icon(Icons.save_rounded),
                label: Text(
                  _isSaving ? 'Enregistrement...' : 'Enregistrer la mesure',
                  style: const TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.w600,
                  ),
