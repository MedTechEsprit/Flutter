...
class _FindPharmaciesScreenState extends State<FindPharmaciesScreen> {
  ...
  Future<void> _openRequestAvailability(_DiabetesMedication medication) async {
    ...
    if (pharmacyOptions.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Aucune pharmacie trouvee a proximite (rayon 25 km).'),
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ChoiceChip(
                            ...
                            selectedColor: AppColors.primaryBlue.withOpacity(0.85),
...
class _ResolvedCoordinates {
  ...
  const _ResolvedCoordinates({required this.latitude, required this.longitude});
}
  ...
  Widget build(BuildContext context) {
    ...
    if (insight == null) {
      ...
      if (hasFirstResponse) {
        ...
        statusText = 'Demande expiree (${cooldownHours}h). Bouton actif.';
      }
    }
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    label: Text(buttonLabel, style: const TextStyle(fontWeight: FontWeight.w800)),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.transparent,
                  ...
                  if ((badge ?? 0) > 0)
                    Positioned(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: const Color(0xFFFF4B4B), // Red badge matching screenshot
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: Colors.white, width: 1.5),
                        ),
                        child: Text(
                          (badge ?? 0) > 99 ? '99+' : badge!.toString(),
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 9,
                            fontWeight: FontWeight.w700,
