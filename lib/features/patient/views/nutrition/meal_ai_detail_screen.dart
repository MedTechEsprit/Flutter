              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  return _buildMessage(
                    'Failed to load AI details: ${snapshot.error}',
                  );
                }
                      ...
                      _sectionCard(
                        ...
                        child: Wrap(
                          ...
                          children: [
                            _metricChip(
                              'Calories',
                              '${detail.analysisResult['calories'] ?? widget.meal.calories?.round() ?? 0}',
                            ),
                            _metricChip(
                              'Carbs',
                              '${detail.analysisResult['carbs'] ?? widget.meal.carbs.round()} g',
                            ),
                            _metricChip(
                              'Protein',
                              '${detail.analysisResult['protein'] ?? widget.meal.protein.round()} g',
                            ),
                            _metricChip(
                              'Fat',
                              '${detail.analysisResult['fat'] ?? widget.meal.fat.round()} g',
                            ),
                            if ((detail.analysisResult['glycemicIndex']
                                    ?.toString()
                                    .isNotEmpty ??
                                false))
                              _metricChip(
                                'GI',
                                detail.analysisResult['glycemicIndex']
                                    .toString(),
                              ),
                          ],
                        ),
