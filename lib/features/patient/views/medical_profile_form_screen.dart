  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Profil médical sauvegardé!'), backgroundColor: Colors.green),
        );
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            widget.isPostRegistration ? 'Complétez votre profil' : 'Modifier le profil médical',
  ...
  Widget _buildStep1() {
    return Column(
      ...
      children: [
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text('IMC calculé: ${bmi.toStringAsFixed(1)}', style: const TextStyle(fontWeight: FontWeight.w600, color: AppColors.softGreen)),
                  ],
                ),
              );
            }
        ...
        _textField(_contactUrgenceController, 'Nom et téléphone', TextInputType.text),
      ],
    );
  }
  ...
  Widget _buildStep4() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        _checkTile('Antécédents familiaux de diabète', _antecedentsFamiliaux, (v) => setState(() => _antecedentsFamiliaux = v!)),
        ...
        _checkTile('Problèmes rénaux', _problemesRenaux, (v) => setState(() => _problemesRenaux = v!)),
        _checkTile('Problèmes oculaires (rétinopathie)', _problemesOculaires, (v) => setState(() => _problemesOculaires = v!)),
        _checkTile('Neuropathie diabétique', _neuropathie, (v) => setState(() => _neuropathie = v!)),
        const SizedBox(height: 16),
        _sectionHeader('Complications actuelles'),
        const SizedBox(height: 8),
        _checkTile('Pied diabétique', _piedDiabetique, (v) => setState(() => _piedDiabetique = v!)),
        _checkTile('Ulcères', _ulceres, (v) => setState(() => _ulceres = v!)),
        _checkTile('Hypoglycémies fréquentes', _hypoglycemiesFreq, (v) => setState(() => _hypoglycemiesFreq = v!)),
        _checkTile('Hyperglycémies fréquentes', _hyperglycemiesFreq, (v) => setState(() => _hyperglycemiesFreq = v!)),
        _checkTile('Hospitalisations récentes', _hospitalisationsRecentes, (v) => setState(() => _hospitalisationsRecentes = v!)),
      ],
    );
  }
  ...
  Widget _buildStep5() {
    return Column(
      ...
      children: [
        ...
        _textField(_allergiesController, 'Ex: Pénicilline, Aspirine (séparées par des virgules)', TextInputType.text),
        const SizedBox(height: 16),
        _fieldLabel('Autres maladies chroniques'),
        _textField(_maladiesChroniquesController, 'Ex: Asthme, Thyroïde (séparées par des virgules)', TextInputType.text),
      ],
    );
  }
